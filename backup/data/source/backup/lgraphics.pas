...
type
  ...
  public
    ...
    property Minimap: TImage read FMinimap write FMinimap;
  end;
...
var
  ...
  cw, ch: Integer;
begin
  cw:=Picture.Bitmap.Canvas.Width;
  ch:=Picture.Bitmap.Canvas.Height;
  if not (dbg or animate) then Visible:=False;
  Picture.Bitmap.Canvas.Clear;
  Picture.Bitmap.BeginUpdate(True);
  if Minimap<>Nil then
  ...
  begin
    ...
    begin
      ...
      begin
        if dbg then Picture.Bitmap.Canvas.Pixels[fullPosition.coords.X, fullPosition.coords.Y]:=clGreen;
        if (Minimap<>Nil) and (X<>0) and (Y<>0) then
        begin
          Minimap.Canvas.Pixels[
            round(Minimap.Canvas.Width+(Minimap.Canvas.Width/(Xmax/X/Y*Ymax))),
            Minimap.Canvas.Height-2*round(Minimap.Canvas.Height/(Ymax/Y))
          ]:=clGreen;
        end;
        if Level.data.Canvas.Pixels[fullPosition.coords.X, fullPosition.coords.Y]=clBlack then
        begin
          if dbg then Picture.Bitmap.Canvas.Pixels[fullPosition.coords.X, fullPosition.coords.Y]:=clRed;
          if (Minimap<>Nil) and (X<>0) and (Y<>0) then
          begin
            Minimap.Canvas.Pixels[
              round(Minimap.Canvas.Width/2+Minimap.Canvas.Width/(Xmax/X/Y*Ymax)),
              Minimap.Canvas.Height-2*round(Minimap.Canvas.Height/(Ymax/Y))
            ]:=clRed;
          end;
