...
type
  ...
  public
    constructor Create(Ax: Extended=0; Ay: Extended=0; ADirection: Extended=0);
    function move(ADirection: TDirection; ASteps: Integer; AUpdate: Boolean): TLPlayerPosition;
...
constructor TLPlayerPosition.Create(Ax: Extended=0; Ay: Extended=0; ADirection: Extended=0);
begin
  inherited Create(Ax, Ay);
  FDirection:=ADirection;
end;

function TLPlayerPosition.move(
...
var dir: TLDirection;
begin
  Result:=TLPlayerPosition.Create(Fx, Fy, direction);
  dir:=Result.direction+ADirection;
  Result.Fx:=Result.Fx+sin(dir)*ASteps;
  Result.Fy:=Result.Fy-cos(dir)*ASteps;
  if AUpdate then Self:=Result;
end;
